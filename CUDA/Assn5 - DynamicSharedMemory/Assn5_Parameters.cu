...
__global__ void transpose_kernel(float* IN, int Size, float* OUT, const int MAX_THREADS, const int TILE_HEIGHT, const int PADDING);
...
void transpose_kernel(float* IN, int n, float* OUT,
                      const int MAX_THREADS, const int TILE_HEIGHT, const int PADDING)
{
...
void host_execute (float* in, int n, float* out)
{
    ...
    transpose_kernel<<<grid, block, sizeof(float)*(TILE_HEIGHT*(TILE_WIDTH + PADDING))
                    >>>(IN, n, OUT, MAX_THREADS, TILE_HEIGHT, PADDING);
